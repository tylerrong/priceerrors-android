 ...
 * menu doubles as an intervention point: the trial offer sits right next to
...
object PriceErrorsShortcuts {
     ...
     * everything, so pitching a trial would be noise — they get no custom menu.
     */
    fun refresh(context: Context, isPro: Boolean) {
        ...
        val intent = Intent(appContext, MainActivity::class.java)
            ...
            .putExtra(EXTRA_SHORTCUT, TRIAL_ID)

        val trial = ShortcutInfoCompat.Builder(appContext, TRIAL_ID)
            .setShortLabel("Try for free")
        ...
        runCatching { ShortcutManagerCompat.setDynamicShortcuts(appContext, listOf(trial)) }
    }
